    ...
    const stackData = {
      en: {
        ...
        balancer: {
          ...
          code: `// Checks if brackets in string are balanced\nbool isValid(string s) {\n  stack<char> st; // Standard C++ Stack container\n  \n  for (char c : s) {\n    // 1. If opening bracket, push to stack\n    if (c == \'(\' || c == \'{\' || c == \'[\') {\n      st.push(c);\n    } \n    // 2. If closing bracket\n    else if (c == \')\' || c == \'}\' || c == \']\') {\n      if (st.empty()) return false; // Empty stack means mismatch\n      \n      // Check if top matches current closing bracket\n      if (c == \')\' && st.top() != \'(\') return false;\n      if (c == \'}\' && st.top() != \'{\') return false;\n      if (c == \']\' && st.top() != \'[\') return false;\n      \n      st.pop(); // Match found, pop from stack\n    }\n  }\n  // 3. If stack is empty at the end, expression is balanced\n  return st.empty();\n}`
        },
        infix: {
          ...
          code: `// Returns operator precedence priority\nint prec(char c) {\n  if (c == \'^\') return 3; // Power has highest priority\n  if (c == \'/\' || c == \'*\') return 2;\n  if (c == \'+\' || c == \'-\') return 1;\n  return -1;\n}\n\n// Converts infix string to postfix string\nstring infixToPostfix(string s) {\n  stack<char> st;\n  string result = ""; // Stores postfix result\n  \n  for (char c : s) {\n    // 1. If operand, add directly to output\n    if (isalnum(c)) {\n      result += c;\n    }\n    // 2. If \'(\', push to stack\n    else if (c == \'(\') {\n      st.push(\'(\');\n    }\n    // 3. If \')\', pop operators to output until \'(\' is found\n    else if (c == \')\') {\n      while (!st.empty() && st.top() != \'(\') {\n        result += st.top();\n        st.pop();\n      }\n      st.pop(); // Pop the \'(\'\n    }\n    // 4. If operator\n    else {\n      // Pop higher or equal precedence operators to output first\n      while (!st.empty() && prec(c) <= prec(st.top())) {\n        result += st.top();\n        st.pop();\n      }\n      st.push(c); // Push current operator to stack\n    }\n  }\n  // 5. Pop all remaining operators in stack to output\n  while (!st.empty()) {\n    result += st.top();\n    st.pop();\n  }\n  return result;\n}`
        },
        evaluator: {
          ...
          code: `// Evaluates postfix string (with space-separated tokens)\nint evaluatePostfix(string exp) {\n  stack<int> st; // Holds operand numbers\n  \n  for (int i = 0; i < exp.length(); i++) {\n    if (exp[i] == \' \') continue; // Skip spaces\n    \n    // 1. If digit, extract full number value\n    if (isdigit(exp[i])) {\n      int val = 0;\n      while (i < exp.length() && isdigit(exp[i])) {\n        val = (val * 10) + (exp[i] - \'0\');\n        i++;\n      }\n      i--;\n      st.push(val); // Push operand\n    }\n    // 2. If operator\n    else {\n      int val1 = st.top(); st.pop(); // 1st pop is Right operand\n      int val2 = st.top(); st.pop(); // 2nd pop is Left operand\n      \n      // Calculate and push result back to stack\n      switch (exp[i]) {\n        case \'+\': st.push(val2 + val1); break;\n        case \'-\': st.push(val2 - val1); break;\n        case \'*\': st.push(val2 * val1); break;\n        case \'/\': st.push(val2 / val1); break;\n      }\n    }\n  }\n  return st.top(); // Final evaluated value\n}`
        }
      },
      bn: {
        basic: {
          ...
          code: `// ============================================\n// পদ্ধতি ১: সাধারণ অ্যারে (মুখস্থ করার জন্য সহজতম)\n// ============================================\nint Stack[100];      // স্ট্যাক ডেটা রাখার অ্যারে\nint Top = 0;         // পরবর্তী পুশ করার পজিশন নির্দেশক\nint MaxSize = 100;   // স্ট্যাকের সর্বোচ্চ সাইজ\n\n// স্ট্যাক খালি কিনা চেক করে\nbool isEmpty() {\n  return (Top == 0);\n}\n\n// স্ট্যাক পূর্ণ কিনা চেক করে\nbool isFull() {\n  return (Top == MaxSize);\n}\n\n// স্ট্যাকের শীর্ষে এলিমেন্ট পুশ করে\nbool push(int Element) {\n  if (isFull()) {\n    cout << "Stack is Full\\n";\n    return false; // Overflow এরর\n  }\n  Stack[Top] = Element; // বর্তমান টপ ইনডেক্সে মান রাখা\n  Top++;                 // টপ ইনডেক্স ১ বাড়ানো\n  return true;\n}\n\n// স্ট্যাকের শীর্ষ থেকে এলিমেন্ট পপ করে\nbool pop() {\n  if (isEmpty()) {\n    cout << "Stack empty\\n";\n    return false; // Underflow এরর\n  }\n  Top--; // টপ ইনডেক্স ১ কমিয়ে এলিমেন্ট সরানো\n  return true;\n}\n\n// শীর্ষ এলিমেন্টের মান রিটার্ন করে\nint topElement() {\n  return Stack[Top - 1];\n}\n\n// ============================================\n// পদ্ধতি ২: অবজেক্ট অরিয়েন্টেড ক্লাস পদ্ধতি\n// ============================================\nclass MyStack {\n  int Stack[100];\n  int Top, MaxSize;\npublic:\n  MyStack(int Size = 100) {\n    MaxSize = Size;\n    Top = 0; // খালি স্ট্যাক শুরু\n  }\n  bool isEmpty() { return (Top == 0); }\n  bool isFull() { return (Top == MaxSize); }\n  bool push(int Element) {\n    if (isFull()) return false;\n    Stack[Top++] = Element;\n    return true;\n  }\n  bool pop() {\n    if (isEmpty()) return false;\n    Top--;\n    return true;\n  }\n  int topElement() { return Stack[Top - 1]; }\n};`
        },
        balancer: {
          title: 'বন্ধনী সমতা পরীক্ষা (Parentheses Balancer)',
          desc: 'স্ট্যাকের একটি বাস্টব প্রয়োগ হলো বন্ধনীর সমতা পরীক্ষা করা। প্রতিটি বন্ধনী চিহ্নের জন্য এটি চেক করে যে বন্ধনীটি স্ট্যাক থেকে পপ হওয়া বিপরীত বন্ধনীর সাথে মেলে কিনা।',
          steps: [
          ...
          code: `// বন্ধনীর সমতা পরীক্ষা করার ফাংশন\nbool isValid(string s) {\n  stack<char> st; // C++ স্ট্যান্ডার্ড স্ট্যাক হেডার\n  \n  for (char c : s) {\n    // ১. শুরুর বন্ধনী হলে স্ট্যাকে পুশ করুন\n    if (c == \'(\' || c == \'{\' || c == \'[\') {\n      st.push(c);\n    } \n    // ২. শেষের বন্ধনী হলে\n    else if (c == \')\' || c == \'}\' || c == \']\') {\n      if (st.empty()) return false; // স্ট্যাক খালি থাকলে অমিল\n      \n      // শীর্ষ বন্ধনী বর্তমান বন্ধনীর সাথে মেলে কিনা দেখুন\n      if (c == \')\' && st.top() != \'(\') return false;\n      if (c == \'}\' && st.top() != \'{\') return false;\n      if (c == \']\' && st.top() != \'[\') return false;\n      \n      st.pop(); // মিলে গেলে পপ করে বাদ দিন\n    }\n  }\n  // ৩. স্ক্যান শেষে স্ট্যাক খালি হলে সমতা সঠিক\n  return st.empty();\n}`
        },
        infix: {
          ...
          code: `// অপারেটরের অগ্রাধিকার নির্ধারণী ফাংশন\nint prec(char c) {\n  if (c == \'^\') return 3; // পাওয়ারের অগ্রাধিকার সবার উপরে\n  if (c == \'/\' || c == \'*\') return 2;\n  if (c == \'+\' || c == \'-\') return 1;\n  return -1;\n}\n\n// Infix থেকে Postfix রূপান্তর করার ফাংশন\nstring infixToPostfix(string s) {\n  stack<char> st;\n  string result = ""; // Postfix ফলাফল জমার স্ট্রিং\n  \n  for (char c : s) {\n    // ১. অপারেন্ড পেলে সরাসরি আউটপুটে যোগ করুন\n    if (isalnum(c)) {\n      result += c;\n    }\n    // ২. শুরুর বন্ধনী '(' পেলে স্ট্যাকে পুশ করুন\n    else if (c == \'(\') {\n      st.push(\'(\');\n    }\n    // ৩. শেষের বন্ধনী ')' পেলে '(' না পাওয়া পর্যন্ত পপ করুন\n    else if (c == \')\') {\n      while (!st.empty() && st.top() != \'(\') {\n        result += st.top();\n        st.pop();\n      }\n      st.pop(); // '(' পপ করে বাদ দিন\n    }\n    // ৪. অপারেটর পাওয়া গেলে\n    else {\n      // সমান বা বেশি অগ্রাধিকারের অপারেটর আগে আউটপুটে পপ করুন\n      while (!st.empty() && prec(c) <= prec(st.top())) {\n        result += st.top();\n        st.pop();\n      }\n      st.push(c); // বর্তমান অপারেটর পুশ করুন\n    }\n  }\n  // ৫. স্ট্যাকের অবশিষ্ট সব অপারেটর আউটপুটে পপ করুন\n  while (!st.empty()) {\n    result += st.top();\n    st.pop();\n  }\n  return result;\n}`
        },
        evaluator: {
          ...
          code: `// পোস্টফিক্স রাশিমালা মূল্যায়নের ফাংশন (স্পেস-সেপারেটেড)\nint evaluatePostfix(string exp) {\n  stack<int> st; // সংখ্যা জমার স্ট্যাক\n  \n  for (int i = 0; i < exp.length(); i++) {\n    if (exp[i] == \' \') continue; // স্পেস এড়িয়ে চলুন\n    \n    // ১. সংখ্যা পেলে পুরো মানটি বের করুন\n    if (isdigit(exp[i])) {\n      int val = 0;\n      while (i < exp.length() && isdigit(exp[i])) {\n        val = (val * 10) + (exp[i] - \'0\');\n        i++;\n      }\n      i--;\n      st.push(val); // অপারেন্ড পুশ করুন\n    }\n    // ২. অপারেটর পাওয়া গেলে\n    else {\n      int val1 = st.top(); st.pop(); // ১ম পপ করা মানটি ডান অপারেন্ড\n      int val2 = st.top(); st.pop(); // ২য় পপ করা মানটি বাম অপারেন্ড\n      \n      // হিসাব করে ফলাফলটি আবার স্ট্যাকে পুশ করুন\n      switch (exp[i]) {\n        case \'+\': st.push(val2 + val1); break;\n        case \'-\': st.push(val2 - val1); break;\n        case \'*\': st.push(val2 * val1); break;\n        case \'/\': st.push(val2 / val1); break;\n      }\n    }\n  }\n  return st.top(); // চূড়ান্ত মূল্যায়িত মান\n}`
        }
      }
    };
